...
export const seedUsers = [
  {
    ...
    role: 'Clinician',
    // Pre-hashed 'MedLens2026!' using bcryptjs (salt rounds 10)
    passwordHash: '$2a$10$wN17Q2X4T8F2Pz69uX6kueS2g3g.XjJ3M7jQ7K0R7k0x8L2k0M2mS',
...
export async function runSeed() {
  ...
  // Seed user with bcrypt hashed password for instant demo login
  const bcrypt = require('bcryptjs');
  const hashedPass = await bcrypt.hash('MedLens2026!', 10);
  await db.collection('users').insertOne({
    ...seedUsers[0],
    passwordHash: hashedPass
  });

  return { success: true, message: 'MongoDB seeded with sample patients, reports, lab results, signals, audit logs, and demo user.' };
}
