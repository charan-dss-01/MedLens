...
import { getGenerativeAIClient, isGeminiAPIKeyConfigured } from './client';
import { SYSTEM_EXTRACTION_PROMPT, SYSTEM_SUMMARY_PROMPT } from './prompts';
...
export async function generateSafePatientSummary(
  ...
  try {
    ...
    const model = genAI.getGenerativeModel({
      model: 'gemini-1.5-flash',
      systemInstruction: SYSTEM_SUMMARY_PROMPT
    });
